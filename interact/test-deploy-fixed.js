...
async function testDeploy() {
  try {
    ...
    const contract = await factory.deploy({
      gasLimit: 3000000,
      gasPrice: 1000000000  // Try with a non-zero gas price
    });
    ...
    // Get the deployed bytecode to verify it was deployed correctly
    const deployedCode = await provider.getCode(address);
    console.log("Deployed code length:", deployedCode.length);
    
    // Test contract interaction with proper encoded data
